...
func TestGenerateOAuthCodeCarriesAffiliateInLoginFlow(t *testing.T) {
	...
	c.Request = httptest.NewRequest(http.MethodPost, "/api/oauth/state", strings.NewReader(`{"provider":"auth-flow-test","intent":"login","aff":"affiliate-code","invite_code":"registration-code"}`))
	...
	assert.Equal(t, "affiliate-code", payload.AffiliateCode)
	assert.Equal(t, "registration-code", payload.InviteCode)
...
func TestGenerateOAuthCodeRejectsInviteForBinding(t *testing.T) {
	setupAuthFlowControllerTest(t)
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/oauth/state", strings.NewReader(`{"provider":"auth-flow-test","intent":"bind","invite_code":"MUST-NOT-BE-CONSUMED"}`))
	c.Request.Header.Set("Content-Type", "application/json")
	c.Set("id", 42)
