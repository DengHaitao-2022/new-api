	...
	if model.IsTelegramIdAlreadyTaken(telegramId) {
		...
		err = model.DB.Transaction(func(tx *gorm.DB) error {
			...
			if common.RegistrationInviteRequired {
				invite, err := model.LockValidRegistrationInviteWithTx(tx, params.Get("invite_code"))
				if err != nil {
					return err
				}
			...
			if err := model.ClaimExternalIdentityWithTx(tx, model.ExternalIdentityProviderTelegram, telegramId, user.Id); err != nil {
				return err
			}
			if common.RegistrationInviteRequired {
				return model.UseRegistrationInviteWithTx(tx, registrationInvite, user.Id, "telegram")
			}
			return nil
		})
		if err != nil {
