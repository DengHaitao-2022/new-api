...
func GenerateOAuthCode(c *gin.Context) {
	...
	flowPayload.InviteCode = request.InviteCode
...
func handleOAuthLogin(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, flow *model.AuthFlow) {
	...
	user, err := findOrCreateOAuthUser(c, provider, oauthUser, payload.AffiliateCode, payload.InviteCode)
	if err != nil {
...
func findOrCreateOAuthUser(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, affiliateCode string, inviteCode string) (*model.User, error) {
	user := &model.User{}
	...
	if genericProvider, ok := provider.(*oauth.GenericOAuthProvider); ok {
		...
		err := model.DB.Transaction(func(tx *gorm.DB) error {
			...
			if common.RegistrationInviteRequired {
				invite, err := model.LockValidRegistrationInviteWithTx(tx, inviteCode)
				if err != nil {
					return err
				}
			...
			if common.RegistrationInviteRequired {
				if err := model.UseRegistrationInviteWithTx(tx, registrationInvite, user.Id, "oauth"); err != nil {
					return err
				}
			}
